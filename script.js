...
(function() {
    ...
    var compareTableContainer = document.getElementById('compareTableContainer');
    ...
    function updateCompareBar() {
        ...
        if (compareList.length > 0) {
            ...
            compareItems.innerHTML = compareList.map(function(name, i) {
            ...
            document.querySelectorAll('.remove-compare').forEach(function(el) {
                el.addEventListener('click', function() {
    ...
    function addCompareCheckboxes() {
        document.querySelectorAll('.property-card').forEach(function(card) {
            ...
            checkboxDiv.style.cssText = 'position:absolute; bottom:15px; left:15px; z-index:5; display:flex; align-items:center; gap:6px; color:#aaa; font-size:12px;';
            checkboxDiv.innerHTML = '<input type="checkbox" class="compare-select" data-name="' + name + '" style="accent-color:#D4AF37; width:16px; height:16px; cursor:pointer;"> <span>Compare</span>';
            card.style.position = 'relative';
            card.appendChild(checkboxDiv);
            
            var chk = checkboxDiv.querySelector('.compare-select');
            chk.addEventListener('change', function() {
                if (this.checked) {
                    ...
                    var index = compareList.indexOf(name);
                    if (index > -1) {
                        compareList.splice(index, 1);
                        localStorage.setItem('compareList', JSON.stringify(compareList));
                        updateCompareBar();
    ...
    document.getElementById('compareBtn')?.addEventListener('click', function() {
        ...
        // Get property data for each selected property
        var compareProperties = [];
        var propertiesData = window.properties || [];
        
        compareList.forEach(function(name) {
            var prop = propertiesData.find(function(p) { return p.name === name; });
            if (prop) {
                compareProperties.push(prop);
                tableHTML += '<th><div class="property-name">' + prop.name + '</div></th>';
            }
        });
        ...
        var rows = [
            ...
            { label: 'Location', key: 'locationDisplay', type: 'text' },
            { label: 'Price', key: 'priceText', type: 'text' },
        ...
        rows.forEach(function(row) {
            tableHTML += '<tr>';
            tableHTML += '<td class="label-cell">' + row.label + '</td>';
            compareProperties.forEach(function(prop) {
                if (row.type === 'image') {
                    tableHTML += '<td><img src="' + prop[row.key] + '" alt="' + prop.name + '" style="width:100%; max-width:180px; height:130px; object-fit:cover; border-radius:12px;"></td>';
                } else {
                    tableHTML += '<td>' + (prop[row.key] || 'N/A') + '</td>';
                }
            });
            tableHTML += '</tr>';
        });
        ...
        // Add Inquire buttons
        tableHTML += '<tr>';
        tableHTML += '<td class="label-cell">Action</td>';
        compareProperties.forEach(function(prop) {
            tableHTML += '<td><a href="contact.html?property=' + encodeURIComponent(prop.name) + '&type=' + prop.type + '" class="btn-luxury" style="display:inline-block; padding:6px 18px; background:#D4AF37; color:#0a0a0a; border-radius:30px; text-decoration:none; font-weight:600; font-size:12px;">Inquire →</a></td>';
        });
        tableHTML += '</tr>';
...
document.getElementById('clearCompare')?.addEventListener('click', function() {
    ...
    // ===== INITIALIZE =====
    updateCompareBar();
    setTimeout(addCompareCheckboxes, 500);

    // Re-run when properties grid updates
